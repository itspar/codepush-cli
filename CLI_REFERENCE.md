# CodePush CLI Reference Guide

A comprehensive reference for all CodePush CLI commands, options, and features.
...
The CodePush CLI uses a powerful binary diff implementation (`bsdiff43`) for creating efficient patch updates. This significantly reduces the update size by sending only the changes between versions.
...
```bash
code-push-itspar create-patch <oldBundle> <newBundle> <outputDir>
...
1. **Generate Patch**
   ```bash
   code-push-itspar create-patch ./v1/bundle ./v2/bundle ./patches/
   ```

2. **Release Patch**
   ```bash
   code-push-itspar release MyApp-iOS ./patches/patch.zip "1.0.0" \
...
```shell
code-push-itspar create-patch <old_bundle> <new_bundle> <patch_directory>
```
...
```shell
code-push-itspar apply-patch <old_bundle> <patch_file> <output_file>
```

Example:
```shell
code-push-itspar apply-patch \
...
```
code-push-itspar register <optional: server-url>
```
...
```shell
code-push-itspar login <optional: server-url>
```
...
```shell
code-push-itspar session ls
code-push-itspar session rm <machineName>
```
...
```shell
code-push-itspar access-key add "VSTS Integration"
```
...
```shell
code-push-itspar login --accessKey <accessKey>
```
...
```shell
code-push-itspar access-key patch <accessKeyName> --name "new name" --ttl 10d
```
...
```
code-push-itspar app add <appName>
```
...
```
code-push-itspar app add MyApp-Android
code-push-itspar app add MyApp-iOS
```
...
```
code-push-itspar app rename <appName> <newAppName>
```
...
```
code-push-itspar app rm <appName>
```
...
```shell
code-push-itspar collaborator add <appName> <collaboratorEmail>
```
...
```shell
code-push-itspar collaborator rm <appName> <collaboratorEmail>
```
...
```shell
code-push-itspar collaborator ls <appName>
```
...
```
code-push-itspar deployment add <appName> <deploymentName>
```

If you want to re-use an existing deployment key, you can do this with:

```
code-push-itspar deployment add <appName> <deploymentName> -k <existing-deployment-key>
```

Just like with apps, you can remove and rename deployments as well, using the following commands respectively:

```
code-push-itspar deployment rm <appName> <deploymentName>
code-push-itspar deployment rename <appName> <deploymentName> <newDeploymentName>
```
...
```
code-push-itspar deployment ls <appName> [--displayKeys|-k]
```
...
2. [React Native](#releasing-updates-react-native) - Performs the same functionality as the general release command, but also handles the task of generating the updated app contents for you (JS bundle and assets), instead of requiring you to run both `react-native bundle` and then `code-push-itspar release`.
...
This specifies the name of the CodePush app that this update is being released for. This value corresponds to the friendly name that you specified when originally calling `code-push-itspar app add` (e.g. "MyApp-Android"). If you need to look it up, you can run the `code-push-itspar app ls` command to see your list of apps.
...
```shell
code-push-itspar release-react <appName> <platform>
...
```shell
code-push-itspar release-react MyApp-iOS ios
```
...
```shell
code-push-itspar release-react MyApp-Android android -p "./foo/bar/"
code-push-itspar release-react MyApp-Android android -p "./foo/bar/build.gradle"
```
...
```shell
code-push-itspar release-react MyApp-iOS ios -p "./foo/bar/MyFile.plist"
```
...
code-push-itspar release-react MyApp-iOS ios --pre "STAGING"

# Tell the CLI to use your dev plist (`DEV-Info.plist`).
# Note that the hyphen separator can be explicitly stated.
code-push-itspar release-react MyApp-iOS ios --pre "DEV-"
```
...
```shell
code-push-itspar patch <appName> <deploymentName>
[--label <releaseLabel>]
...
code-push-itspar patch MyApp-iOS Production -m

# Increase the rollout for v23 to 50%
code-push-itspar patch MyApp-iOS Production -l v23 -rollout 50%
```

### Label parameter

Indicates which release (e.g. `v23`) you want to update within the specified deployment. If ommitted, the requested changes will be applied to the latest release in the specified deployment. In order to look up the label for the release you want to update, you can run the `code-push-itspar deployment history` command and refer to the `Label` column.
...
code-push-itspar patch MyApp-iOS Staging -t "1.0.0 - 1.0.5"
```
...
```
code-push-itspar promote <appName> <sourceDeploymentName> <destDeploymentName>
[--description <description>]
[--label <label>]
[--disabled <disabled>]
[--mandatory]
...
code-push-itspar promote MyApp-iOS Staging Production -t "*"
```
...
```
code-push-itspar rollback <appName> <deploymentName>
code-push-itspar rollback MyApp-iOS Production
```
...
```
code-push-itspar rollback MyApp-iOS Production --targetRelease v34
```
...
```
code-push-itspar deployment history <appName> <deploymentName>
```
...
```
code-push-itspar deployment clear <appName> <deploymentName>
```
...
```shell
code-push-itspar release-react <appName> <platform> --privateKeyPath private.pem
```
