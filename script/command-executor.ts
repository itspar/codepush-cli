  ...
  return Q(<void>null).then(() => {
    switch (command.type) {
      ...
      default:
        ...
        if (!connectionInfo) {
          throw new Error(
            "You are not currently logged in. Run the 'code-push-itspar login' command to authenticate with the CodePush server."
          );
        }
...
function sessionRemove(command: cli.ISessionRemoveCommand): Promise<void> {
  if (os.hostname() === command.machineName) {
    throw new Error("Cannot remove the current login session via this command. Please run 'code-push-itspar logout' instead.");
  } else {
    return confirm().then((wasConfirmed: boolean): Promise<void> => {
      if (wasConfirmed) {
...
function createPatch(command: cli.ICreatePatchCommand): Promise<void> {
  return Q.Promise<void>((resolve, reject) => {
    const scriptPath = path.join(__dirname, CodePushConfigConstants.SCRIPT_DIRECTORY, CodePushConfigConstants.CREATE_PATCH_SCRIPT);
...
function applyPatch(command: cli.IApplyPatchCommand): Promise<void> {
  return Q.Promise<void>((resolve, reject) => {
    const scriptPath = path.join(__dirname, CodePushConfigConstants.SCRIPT_DIRECTORY, CodePushConfigConstants.APPLY_PATCH_SCRIPT);
